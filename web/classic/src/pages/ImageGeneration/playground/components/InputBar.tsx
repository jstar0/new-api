...
export default function InputBar() {
  ...
  const handleToggleFavorite = useCallback(() => {
    ...
    const allFavorite =
      selectedTasks.length > 0 && selectedTasks.every((t) => t.isFavorite)
  ...
  const moderationDisabled =
    activeProfile.apiMode === 'responses' || isFalProvider
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (showAtImageMenu) {
      ...
      if ((e.key === 'Enter' && !e.shiftKey) || e.key === 'Tab') {
        e.preventDefault()
        selectAtImageOption(
          atImageOptions[atImageMenuIndex]?.index ?? atImageOptions[0].index
        )
        return
      }
      if (e.key === 'Escape') {
    ...
    <div className={`grid ${cols} flex-1 gap-2 text-xs`}>
      ...
      <label className='relative flex flex-col gap-0.5'>
        ...
        <input
          ...
          className='rounded-xl border border-gray-200/60 bg-white/50 px-3 py-1.5 text-xs shadow-sm transition-all duration-200 focus:outline-none dark:border-white/[0.08] dark:bg-white/[0.03]'
        />
  ...
  return (
    <>
      ...
      <div
        data-input-bar
        className='fixed bottom-4 left-1/2 z-30 w-full max-w-4xl -translate-x-1/2 px-3 transition-all duration-300 sm:bottom-6 sm:px-4'
      >
        ...
        <div
          ref={cardRef}
          className='rounded-2xl border border-white/50 bg-white/70 p-3 shadow-[0_8px_30px_rgb(0,0,0,0.08)] ring-1 ring-black/5 backdrop-blur-2xl sm:rounded-3xl sm:p-4 dark:border-white/[0.08] dark:bg-gray-900/70 dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] dark:ring-white/10'
        >
          {/* 移动端拖动条 */}
          <div
            ref={handleRef}
            className='-mt-1 flex cursor-pointer touch-none justify-center pt-0.5 pb-2 sm:hidden'
          ...
          <div className='relative'>
            ...
            <div
              ...
              className='min-h-[42px] w-full rounded-2xl border border-gray-200/60 bg-white/50 px-4 py-3 text-sm leading-relaxed break-words whitespace-pre-wrap shadow-sm transition-[border-color,box-shadow] duration-200 outline-none empty:before:pointer-events-none empty:before:text-gray-400 empty:before:content-[attr(data-placeholder)] focus:ring-1 focus:ring-blue-300/40 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-100 dark:empty:before:text-gray-500 dark:focus:ring-blue-500/30'
            />
          </div>
            ...
            <div className='hidden items-end justify-between gap-3 sm:flex'>
              ...
              <div className='mb-0.5 flex flex-shrink-0 gap-2'>
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      hasSubmitApiConfig ? submitTask() : setShowSettings(true)
                    }
