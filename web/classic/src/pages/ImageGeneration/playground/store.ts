...
function countSuccessfulOutputImages(tasks: TaskRecord[]) {
  return tasks.reduce(
    (count, task) =>
      count + (task.status === 'done' ? task.outputImages.length : 0),
    0
  )
}
...
export function getPersistedState(state: AppState) {
  ...
  return {
    ...
    ...(settings.persistInputOnRestart
      ? {
          prompt: state.prompt,
          inputImages: state.inputImages.map((img) => ({
            id: img.id,
            dataUrl: '',
          })),
        }
...
export const useStore = create<AppState>()(
  persist(
    ...
    {
      name: 'turboapi-image-playground',
...
function scheduleOpenAIWatchdog(taskId: string, timeoutSeconds: number) {
  ...
  const timer = setTimeout(() => {
    ...
    const failed = failOpenAITaskIfStillRunning(
      taskId,
      createOpenAITimeoutError(timeoutSeconds)
    )
  ...
  return (
    normalized.profiles.find(
      (profile) =>
        ...
        (profile.name === task.apiProfileName ||
          profile.model === task.apiModel)
    ) ??
...
export function getTaskApiProfile(
  ...
  const candidates = normalized.profiles.filter(
  ...
  if (task.apiProfileName) {
    const byName = candidates.find(
      (profile) => profile.name === task.apiProfileName
    )
  ...
  if (task.apiModel) {
    const modelMatches = candidates.filter(
      (profile) => profile.model === task.apiModel
    )
...
export async function removeMultipleTasks(taskIds: string[]) {
  const { tasks, setTasks, inputImages, showToast, selectedTaskIds } =
    useStore.getState()
